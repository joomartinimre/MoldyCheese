...
const tagItems = [
  "Gimnázium", "Egyetem", "Általános Iskola", "Technikum",
  "Gyakorló Gimnázium", "+Kollégium", "Gyakorló Általános Iskola",
  "Kísérleti Gimnázium", "Kísérleti Általános Iskola"
];
...
const fetchPlaces = async (sort = "visits"): Promise<Place[]> => {
  ...
  try {
    ...
    const response = await axios.get(`${API_BASE}/school?sort=${sort}&${tagParams}`);
...
<div v-else class="layout-wrapper" :style="layoutWrapperStyle">
  ...
  <v-container>
    <v-card max-width="1700px" style="margin-left: auto; margin-right: auto;">
      <v-card-title style="white-space: normal;" v-if="!mobile" class="text-h4">Magyarország iskolái (2025)</v-card-title>
      <v-card-title style="white-space: normal;" v-if="mobile" class="text-h5">Magyarország iskolái (2025)</v-card-title>
