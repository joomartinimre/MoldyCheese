...
const fetchPlaces = async (sort = "visits") => {
  try {
    ...
    const popularResponse = await axios.get(`${API_BASE}/school?sort=${sort}&${tagParams}`);
...
<div class="layout-wrapper" :style="layoutWrapperStyle">
  ...
  <v-container>
    <v-card max-width="1700px" style="margin-left: auto; margin-right: auto;">
      <v-card-title v-if="!mobile" class="text-h4">Magyarország iskolái (2025)</v-card-title>
      <v-card-title v-if="mobile" class="text-h5">Magyarország iskolái (2025)</v-card-title>
